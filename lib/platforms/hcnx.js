...
var Hcnx = new ChatExpress({
  ...
  routes: {
    ...
    //       if (results.length === 0){
    //         connection.execute(
    //           'INSERT INTO `mo_logs` SET `sent_by` = ?, `step` = ?, `sms_match` = ?',
    //           [req.body.FROM, 0, 0],
    //           function(err, results, fields) {
    ...
    //         sms_match = results[0].sms_match;
    //       }
    //
    //       var json = { sender:req.body.FROM, text:req.body.MESSAGE, step:step, spamtype:spamtype, sms_match:sms_match }
    ...
    //
    // },
    // '/redbot/hcnx/satisfaction': function(req, res) {
    '/redbot/hcnxfr': function(req, res) {
      console.log('ROUTE satisfaction');
      const chatServer = this;
      console.log(chatServer);
      // console.log('HCNX /redbot/hcnx/satisfaction');
      // console.log(chatServer);
      var json = { sender:req.body.FROM, text:req.body.MESSAGE }
      json.custom = req.body
      chatServer.receive(json);
      res.send('MT\n\n');
    },
  ...
  routesDescription: {
    ...
    '/redbot/hcnx/satisfaction' : 'Use this for USECASE 3 – Campaign SMS / API sending / Conversational flow with end session management Scenario MKT',
  ...
  webHookScheme: function() {
    console.log('webHookScheme TODO');
