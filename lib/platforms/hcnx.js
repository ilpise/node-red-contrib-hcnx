...
var Hcnx = new ChatExpress({
  ...
  routes: {
    ...
    '/redbot/hcnxfr': function(req, res) {
      ...
      connection.execute(
        ...
        function(err, results, fields) {
          ...
          if (results.length === 0){
            ...
            step = 1+ parseInt(results[0].step)
            // DON'T UPDATE here update only if we have finisced the remaining flow
            // connection.execute(
            //   'UPDATE `mo_logs` SET `step` = ? WHERE `sent_by` = ?',
            //   [step, req.body.FROM],
            //   function(err, results, fields) {
            //     console.log("UPDATE array results");
            //     console.log(results); // results contains rows returned by server
            //     // ResultSetHeader {
            //     //   fieldCount: 0,
            //     //   affectedRows: 1,
      ...
      var json = { sender: req.body.FROM, text: req.body.MESSAGE, step: step }
